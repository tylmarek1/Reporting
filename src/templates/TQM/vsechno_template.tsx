...
import { UnitTable, TotalUnitRow, CreatedBy, Truck, Received, TotalUsedUnitRow } from '../components/footer';
...
const DataHeaderRow = () => (
  <View fixed style={[styles.tableRow, styles.table]}>
    ...
    <DataHeader flexV={1} item="Expiration date"/>
    ...
    <Fragment>
      {items.map(item => (
        <View wrap={false} key={`dataItemRow_${item.orderCode}`} style={styles.tableRow}>
          <DataNormal flexV={1} breakeLength={10} item={item.contract} />
          <DataNormal flexV={1} breakeLength={10} item={item.orderCode} />
          <DataNormal flexV={1} breakeLength={10} item={item.sscc} />

          <DataNormal flexV={1} breakeLength={10} item={item.itemCode} />
          <DataNormal flexV={1} breakeLength={10} item={item.itemExternalProductCode} />
          <DataNormal flexV={1} breakeLength={10} item={item.batchCode} />
          <DataDate flexV={1} item={item.productionDate} />
          <DataDate flexV={1} item={item.expirationDate} />

          <DataNormal flexV={3} breakeLength={35} item={item.itemDescription} />
